...
#     DatasetUser['TempsQuizz_{}'.format(n)] =
#                                       pd.Series(index=DatasetUser.index)
...
class StudentNet(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size()[0]
        y = F.softmax(self.L1(x))
        y = F.softmax(self.L2(y))
        y = self.drop(y)
        y = self.Sig(self.L3(y))
        # y = y.mean(dim=2)
        return y
...
class StudentDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return Variable(torch.FloatTensor([X[index]]), requires_grad=True),\
            Variable(torch.FloatTensor([y[index]/100]), requires_grad=False)
...
for i_epoch in range(nb_epoch):
    ...
    for batch in train_loader:
        ...
        print("targ", type(targets))
        print("sdfg", pred.size())
        print("azert", targets.size())
